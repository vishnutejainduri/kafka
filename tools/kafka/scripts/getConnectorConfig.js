...
async function callGetConnectorConfig(kubeHost, token, connectorName, pathStart) {
    const options = {
...
async function getConnectorConfig(kubeParams, connectorName) {
    const token = await retry(getSessionToken)(kubeParams);
    const { body, statusCode } = await retry(callGetConnectorConfig)(kubeParams.host, token, connectorName, kubeParams.pathStart);
