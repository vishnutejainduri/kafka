...
const fixEndDate = (product) => {
  product.masterData.current.masterVariant.prices = product.masterData.current.masterVariant.prices.map(price => {
    if (price.custom && price.custom.fields.priceType === 'temporaryMarkdown' && (new Date(price.validUntil)).getSeconds() === 0) {
      price.validUntil = new Date((new Date(price.validUntil)).getTime() - 2000)
    }
    return price;
  })
  product.masterData.current.variants = product.masterData.current.variants.map(currentVariant => {
    currentVariant.prices = currentVariant.prices.map(price => {
      if (price.custom && price.custom.fields.priceType === 'temporaryMarkdown' && (new Date(price.validUntil)).getSeconds() === 0) {
        price.validUntil = new Date((new Date(price.validUntil)).getTime() - 2000)
      }
      return price;
...
const resetOnSale = ({ client, requestBuilder }, productBefore) => {
  const setOnSaleFlag = {
  ...
  const body = JSON.stringify({ version: productBefore.version, actions: [setOnSaleFlag] });

  return client.execute({ method: 'POST', uri, body })
    .then(result => {
      console.log('result', result.body.key);
...
const sendUpdateActions = ({ client, requestBuilder }, actions, productBefore) => {
  ...
  console.log('key', productBefore.key);
...
const deleteProductData = async (ctHelpers, { priceDeleter, onSaleFlagResetter, endDateFixer }, whereQuery) => {
  const method = 'GET';
  ...
  while (resultCount === 500) {
    ...
    if (!lastId) {
      if (whereQuery) {
        ...
        uri = requestBuilder.products.byKey('20034563').build();
      }
    } else {
      if (whereQuery) {
    ...
    try {
      ...
      let syncActions = results.map ((result) => {
        ...
        if (onSaleFlagResetter) {
          return [resetOnSale(ctHelpers, productAfter)]
        }
      });
...
const fixEndDates = (ctHelpers) => deleteProductData(ctHelpers, { endDateFixer: fixEndDate })
const resetOnSaleFlags = (ctHelpers) => deleteProductData(ctHelpers, { onSaleFlagResetter: resetOnSale })
