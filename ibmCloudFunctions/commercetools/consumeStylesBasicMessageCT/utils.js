...
const updateStyleOutlet = async (ctHelpers, productTypeId, stylesBasicMessage) => {
    ...
    let existingCtStyle = await getExistingCtStyle(stylesBasicMessage.id, ctHelpers);

    if (!existingCtStyle) {
      existingCtStyle = (await createAndPublishStyle ({ id: stylesBasicMessage.id, name: { 'en-CA': '', 'fr-CA': '' } }, { id: productTypeId }, null, ctHelpers)).body;
    }
